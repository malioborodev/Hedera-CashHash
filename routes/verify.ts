...
verifyRouter.post('/buyer-ack', async (req, res) => {
  ...
  try {
    await HederaOps.publishLifecycleEvent('BUYER_ACK', invoiceId, { buyerId });
  } catch {}

  return res.json(ok(updated));
});

verifyRouter.post('/attester-sign', async (req, res) => {
  ...
  try {
    await HederaOps.publishLifecycleEvent('ATTESTER_SIGN', invoiceId, { attesterId });
  } catch {}
